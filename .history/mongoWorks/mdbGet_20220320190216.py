...
def colFindID():
    idList = [str(x).split("'",4)[3] for x in db.col.find({},{"_id":1}).limit(5)]
    return(idList)
    
    
def maxRunWorkerTask(ID=None):
    #TRANSFORMATION FUNC
    QUERY = db.client.get_database("local").get_collection("STP").find({ 'match' : {'_id': ID}})     
    for x in QUERY:
        print(x)
